...
function Combat:stopAllModes()
    ...
    self.state.flags.lockMovement = false
...
function Combat:shootTarget(target, opts)
    ...
    if self.state.flags.inCombat then
        self.state.flags.abortCombat = true
        repeat RunService.Heartbeat:Wait() until not self.state.flags.inCombat or not self.state.playable
        if self.state.flags.inCombat then return end
    end
    ...
    self.state.flags.lockMovement = true
    ...
    while root and target and target.Character and not isKO(target) and not self.state.flags.abortCombat and
        self.state.playable and tick() < deadline do
...
function Combat:knock(target)
    if not target then return end
    if self.state.modes.combat ~= "loopknock" and self.state.modes.combat ~= "loopkill" and
        self.state.modes.combat ~= "akill" and self.state.modes.combat ~= "aura" and
        self.state.modes.combat ~= "sentry" and self.state.modes.combat ~= "bsentry" then
...
function Combat:kill(target)
    if not target then return end
    if self.state.modes.combat ~= "loopkill" and self.state.modes.combat ~= "loopknock" and
        self.state.modes.combat ~= "akill" and self.state.modes.combat ~= "aura" and
        self.state.modes.combat ~= "sentry" and self.state.modes.combat ~= "bsentry" then
        self.state:setCombatMode("combat")
    end
...
function Combat:queueAction(fn)
    if self.state.flags.inCombat then return false end
    local thread = task.spawn(fn)
    if self.connections and self.connections.addTask then
        self.connections:addTask(thread)
    end
    return true
end

function Combat:onHeartbeat(playable)
    ...
    -- movement: follow/void/stay (skipped when locked)
    local lp = Players.LocalPlayer
    local char = getChar(lp)
    local root = getRoot(lp)
    if not self.state.flags.lockMovement then
    ...
    if self.state.modes.combat == "loopkill" and self.state.flags.loopkillTarget then
        ...
        if isKO(tgt) then
            if self:queueAction(function() self:kill(tgt) end) then
                self.state.flags.loopkillTarget = nil
                self.state:setCombatMode("idle")
            end
            return
    ...
    elseif self.state.modes.combat == "loopknock" and self.state.flags.loopknockTarget then
        ...
        if self:queueAction(function() self:knock(tgt) end) then
            return
        end
    end
        ...
        if now - (self.timers.aura or 0) > 0.25 then
            ...
            for _, plr in ipairs(Players:GetPlayers()) do
                if plr ~= lp and not isKO(plr) and not self.state:isWhitelisted(plr.Name) and not self.state.auraWhitelist[normalize(plr.Name)] then
                    if self:queueAction(function() self:kill(plr) end) then
                        return
                    end
                end
            end
    ...
    if self.state.modes.combat == "akill" and self.state.flags.akill then
        ...
        if target and part then
            if self:queueAction(function() self:kill(target) end) then
                return
            end
        end
    end
    ...
    if now - (self.timers.assist or 0) > 0.6 then
        ...
        for _, target in pairs(self.state.assistTargets) do
            if target and target.Parent then
                ...
                if threat and self:queueAction(function() self:kill(threat) end) then
                    return
                end
            end
        end
