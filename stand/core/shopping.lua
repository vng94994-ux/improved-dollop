...
function Shopping.new(state, connections)
    local self = setmetatable({
        ...
        feature = {
            click = fireclickdetector ~= nil,
            vim = VirtualInputManager and VirtualInputManager.SendMouseButtonEvent ~= nil,
        },
        cache = {},
...
local function teleportAndInteract(self, head, detector, tries, delay)
    ...
    for _ = 1, tries do
        success = self:interact(head, detector) or success
        task.wait(delay)
    end
...
function Shopping:interact(head, detector)
    if not detector then return false end
    local cam = Workspace.CurrentCamera
    local useVim = self.feature.vim and cam ~= nil
    local useClick = self.feature.click
    if not useVim and not useClick then return false end

    if useVim and head then
        local ok = pcall(function()
            local lookAt = head.Position + Vector3.new(0, 0.2, 0)
            local originalCF = cam.CFrame
            cam.CFrame = CFrame.lookAt(cam.CFrame.Position, lookAt)
            local point = cam:WorldToViewportPoint(lookAt)
...
function Shopping:buy(modelName, tries, delay)
    if not (self.feature.vim or self.feature.click) then return false end
    local detector, head = self:resolve(modelName)
    if not detector or not head then return false end
    return teleportAndInteract(self, head, detector, tries or 8, delay or 0.12)
end
